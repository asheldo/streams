...
import org.asheldo.streams.model.FeedInput;
...
public class StreamsApp {
    ...
    public void run(final String bucketName,
        ...
        FeedInput feedInput = readFeedInput(s3objectResponse, objectMapper);
        LastExposureSettingsImpl lastExposureSettings = feedInput.getLastExposureSettings();
        String channel = feedInput.getChannel();
    ...
    private FeedInput readFeedInput(final InputStream s3objectResponse,
                                    final ObjectMapper objectMapper) throws IOException {
        File inputTemp = File.createTempFile("feedInput", "");
        FileUtils.copyInputStreamToFile(s3objectResponse, inputTemp);
        //        FeedInput feedInput = objectMapper.readValue(s3objectResponse, FeedInput.class);
        FeedInput feedInput = objectMapper.readValue(inputTemp, FeedInput.class);
        return feedInput;
    }
